...
Cargo.Core = {
    ...
    KeyboardShortcut: {
        ...
        InitDefaults: function() {
            this.Add("J", 74, function() {
                Action.Project.Next()
            });
            this.Add("K", 75, function() {
                Action.Project.Prev()
            });
            this.Add("R", 82, function() {
                Action.Project.Random()
            });
            this.Add("X", 88, function() {
            ...
            this.Add("Shift+G", 71, function() {
                Cargo.Helper.GoToUrl("images");
            });
            ...
            this.Add("Shift+H", 72, function() {
                Cargo.Helper.GoToUrl("admin/html");
            });
            this.Add("Shift+C", 67, function() {
                Cargo.Core.loadInspector3();
            });
            this.Add("Shift+D", 68, function() {
                Cargo.Helper.GoToUrl("admin/designs");
            });
            this.Add("Shift+X", 88, function() {
                Cargo.Helper.GoToUrl("");
            });
            this.Add("Shift+I", 73, function() {
                loadInspector();
            });
        ...
        Loader: {
            init: function() {
                ...
                function checkPixelRatio() {
                    ...
                    $.cookie('userPixelRatio', window.devicePixelRatio, {
                        path: '/'
                    });
                }

                function disableHiRes() {
                    $.post("/dispatch/cargosite/cargoUseHiRes", {
                        use: false
                    }, function() {}, "json");
                }
...
Cargo.o.api = Backbone.View.extend({
    ...
    GetSingleProject: function(purl, collection) {
         ...
         */
        var project_model = collection.findWhere({
            project_url: purl
        });
        var page_model = Cargo.Collection.Pages.findWhere({
            project_url: purl
        });
    ...
    GetRandomProject: function() {
        Cargo.Model.Project.fetch({
            ...
            success: function(model) {
                ...
                Cargo.Router.navigate(Cargo.Model.Project.GetProjectUrl(), {
                    trigger: false
                });
            }
        });
...
Cargo.o.helpers = Backbone.View.extend({
    ...
    FindCollectionByProjectUrl: function(project_url) {
        ...
        _.each(Cargo.Collection.Page, function(collection) {
            if (!found_collection) {
                ...
                var model = dummy_collection.where({
    ...
    GetCurrentModelFromProjects: function() {
        return Cargo.Collection.Projects.where({
            project_url: Cargo.Helper.GetCurrentProjectUrl()
        });
    },
    ...
    GetNextProjectModel: function() {
        return Cargo.Collection.Projects.findWhere({
            project_url: Cargo.Helper.GetNextProjectUrl()
        });
    },
    ...
    GetPidFromPurl: function(project_url) {
        // Set the new model
        var model = Cargo.Collection.Projects.where({
            project_url: project_url
        });
    ...
    GetPrevProjectModel: function() {
        return Cargo.Collection.Projects.findWhere({
            project_url: Cargo.Helper.GetPrevProjectUrl()
        });
    },
    ...
    GetTotalProjectsInMemory: function() {
        ...
        var stored_count = 0;
        $.each(Cargo.Collection.Page, function() {
            stored_count += this.length;
        });
...
Cargo.Core.Handlebars = {
    ...
    RegisterHelper_Asset: function() {
        Handlebars.registerHelper('asset', function(options) {
            ...
            var model = Cargo.Collection.Assets.where({
                filename: asset
            })[0];
...
Cargo.Core.Video = {
    ...
    ForceHTML5: function() {
        ...
        if (this.Config.force_HTML5) {
            $.cookie('cargo.video.forcedHTML5', true, {
                expires: 30,
                path: '/'
            });
            this.Config.has_flash_plugin = false;
            ...
            $.cookie('cargo.video.forcedHTML5', null, {
                path: '/'
            });
        ...
        player.children('video').each(function(index, video) {
            $(video).bind('volumechange', function() {
                ...
                if (muted) {
                    ...
                    $.cookie('cargo.video.muted', true, {
                        expires: 30,
                        path: '/'
                    });
                } else {
                    $.cookie('cargo.video.muted', null, {
                        path: '/'
                    });
                }

                $.cookie('cargo.video.volume', volume, {
                    expires: 30,
                    path: '/'
                });
            });
...
Cargo.Core.SocialMedia = {
    ...
    GetFormattedHTML: function(pid) {
        ...
        var project_model = Cargo.Collection.Projects.where({
            id: pid
        })[0];
...
Cargo.Core.Inspector3 = {
    ...
    load: function(customOptions) {
        ...
        $.post("/dispatch/admin/authenticateInspector", {
            url: (Cargo.Helper ? Cargo.Helper.GetUrl() : Cargo.Location.GetCargoUrl())
...
Cargo.o.model.project = Backbone.Model.extend({
    ...
    parse: function(resp, options) {
        var project_model = Cargo.Collection.Projects.findWhere({
...
Cargo.o.view.Main = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        if (!Cargo.Helper.IsAdminEdit() && typeof Mobile != "object") {
            ...
            Cargo.View.Following = new Cargo.o.view.Following({
                el: "body"
            });
        }
...
Cargo.o.view.ProjectDetail = Backbone.View.extend({
    ...
    SaveContent: function(content) {
        ...
        var collection_model = collection.where({
            id: model.get('id')
        })[0];

        if (collection_model) {
            collection_model.set('content', content, {
                silent: true
            });
        }
    ...
    IncrementView: function() {
        ...
        setTimeout(function() {
            if (self.collection.GetId() != "none") {
                $.post('/dispatch/project/addview', {
                    pid: self.collection.GetId()
                });
            }
        }, 500);
    },
                        ...
                        if (Cargo.API.Config.is_domain) {
                            $.post(
                                "/dispatch/cargosite/cargoToolsetLocal", {
                                    token: data.jdata.token,
                                    url: data.jdata.url,
                                    muid: data.jdata.muid,
                                    cv: data.jdata.cv
                                },
                                function(dataObj) {
...
Cargo.app.Rail.View.Master = Backbone.View.extend({
    ...
    Hide: function() {
        ...
        _.delay(function() {
            Cargo.app.Rail.Events.trigger("following:hide")
        }, 300);
    }
...
Cargo.app.Rail.View.Selections = Backbone.View.extend({
    ...
    HandleImageDrop: function() {
        ...
        $.post("/dispatch/selections/selection", {
            mid: this.collection.get("mid"),
            cuid: cuid
...
! function() {
    ...
    a(Node, "saveable", !1), a(Node, "setSaveable", function(a) {
        return this.saveable = a, this
    }), a(Node, "isSaveable", function() {
